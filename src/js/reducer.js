...
const initialState = (attributes: Attributes, defaultDiscount: number, url: string, width: number, email: string): State => ({
  show: false,
  message: null,
  email,
  step: 'product',
...
const getInitialState = (settings: Settings, url: string, width: number, email: string): State => {
  ...
  return initialState(product.attributes, value, url, width, email);
};

export default (prevState: ?State, action: Action): State => {
  const state = prevState || initialState([], 0, '', 1024, '');
  switch (action.type) {
    case Types.init:
